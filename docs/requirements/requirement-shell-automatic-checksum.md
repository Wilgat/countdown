...
| **In-repo publisher SSOT** | Companion file **`countdown.sha256`** (bare SHA-256 hex of `./countdown`) **MUST** ship next to the installable script for the release channel. |
| **Regenerate on change** | After every edit to `./countdown` that is published, regenerate `countdown.sha256` so automatic mode can match. |
...
| **Outside payload** | Pin is env/operator/CI for that process — **MUST NOT** be embedded inside `./countdown` as a self-hash of that file. |
...
1. Hash of `./countdown` stored **inside** `./countdown` as the verify target.  
...
1. Product root **`README.md` MUST** explain **automatic** checksum as the default: algorithm (SHA-256), companion URL pattern, in-repo `countdown.sha256`, match / mismatch / missing outcomes.  
...
| **Channel SSOT** | `SCRIPT_URL` default `https://raw.githubusercontent.com/${REPO_USER}/${REPO_NAME}/main/${APP_NAME}` → `https://raw.githubusercontent.com/Wilgat/countdown/main/countdown` |
| **Companion URL** | `${SCRIPT_URL}.sha256` → `https://raw.githubusercontent.com/Wilgat/countdown/main/countdown.sha256` |
| **In-repo companion** | `countdown.sha256` (bare 64-char hex) |
...
2. Embed the expected SHA-256 of `./countdown` **inside** `./countdown` as verification.  
...
5. Document same-origin `CHECKSUM=$(curl …/countdown.sha256)` as higher assurance than automatic companion verification.  
...
| `./countdown` | Ship unit implementation |
| `countdown.sha256` | In-repo companion digest |
