...
This requirement is the **project Single Source of Truth** for **all CLI output** of the countdown POSIX shell tool: human messages, machine JSON, channel split (stdout vs stderr), and mode behavior (normal / quiet / JSON / debug).
...
3. **Capture pattern for agents/CI:**  
   `countdown --json <cmd> 2>err.log` → stdout = JSON; stderr = diagnostics as mode allows.  
...
Output-related work for countdown is **not done** if any of the following fail:
