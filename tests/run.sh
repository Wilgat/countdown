#!/bin/sh
# =============================================================================
# tests/run.sh — CI entrypoint for countdown (PM-SHELL-CLI-SUITE-TEST-PLAN)
# =============================================================================
# Order: CLI → install lifecycle → online curl (local) → domain
# Assert labels use TP-IDs (TP-CLI / TP-LC / TP-CSUM / TP-U / TP-CURL / TP-COUNTDOWN).
...
# shellcheck source=test_countdown_domain.sh
. "${TESTS_ROOT}/test_countdown_domain.sh"
...
_cleanup() {
    ...
    ci_cleanup_countdown_domain 2>/dev/null || true
    ci_cleanup_env 2>/dev/null || true
}
trap _cleanup EXIT INT HUP TERM

printf 'countdown CI tests\n'
printf 'script: %s\n' "${SCRIPT}"
