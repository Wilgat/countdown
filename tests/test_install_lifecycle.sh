...
run_test_install_lifecycle() {
    ...
    _app_bin="${CI_USER_BIN}/countdown"
    ...
    mkdir -p "${_global_bin}"
    cp "${SCRIPT}" "${_global_bin}/countdown"
    chmod +x "${_global_bin}/countdown"
    _out=$(
    ...
    # Ensure only countdown in USER_BIN so cleanup path can strip rc lines
    find "${CI_USER_BIN}" -mindepth 1 ! -name 'countdown' -exec rm -rf {} + 2>/dev/null || true
