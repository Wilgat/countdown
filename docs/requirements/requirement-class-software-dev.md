...
**Status**: Active (Version 1.0.0 – countdown class law + residual stack)  
**Area**: class  
...
Declare this workspace as a **software-development** project class and hold the **residual collection** of software-engineering stack facts **not already owned** by more specific Active `requirement-shell-*.md` or `requirement-domain-countdown.md` peers: primary language, toolchain policy, package/test tooling, and runtime OS family.
...
|-------|---------------|
| **Project display name** | `countdown` (product root `README.md` H1 SSOT) |
...
| **Test runner** | `tests/run.sh` + `tests/test_cli.sh` + `tests/test_install_lifecycle.sh` + `tests/test_countdown_domain.sh` (POSIX shell) |
...
| Residual stack not duplicating peers | peer shell + domain REQs own pillars | have |
