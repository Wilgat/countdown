...
This requirement is the **project Single Source of Truth** for the **POSIX shell CLI interface** of the countdown tool: command surface, privilege typing, global flags, dispatcher behavior, output modes, and interactive vs non-interactive rules.
...
| **Version SSOT** | `VERSION` in script config block (product SSOT; currently `VERSION="1.1.6"`) |
...
| `about` | Type 0 | `app_about` | Diagnostics: install presence, global/local paths, user, shell, TTY, **cache folder (preferred/fallback/chosen)** and **persistence folder**; JSON when `--json` includes `cache_preferred` / `cache_fallback` / `persistence_storage` / `effective_storage`; **no `CHECKSUM` field**. Folder law: `requirement-shell-cli-storage.md` |
...
| `--persist` | Domain storage mode; routed here, owned by `requirement-domain-countdown.md`. Writes the **persistence folder**, not the cache folder. |

#### Dispatcher acceptance criteria (this project)

1. Unknown token after flag parse → `out_die` with pointer to `countdown help`.  
...
This requirement is satisfied for the countdown shell CLI when all of the following hold:
