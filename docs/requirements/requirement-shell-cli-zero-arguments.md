...
| **Installed (local)** | Managed binary at the user path (`USER_BIN` / `${HOME}/.local/bin/countdown`) |
| **Installed (global)** | Managed binary at the global path (`GLOBAL_BIN` / `/usr/local/bin/countdown`) |
...
| Term | Definition for countdown |
|------|----------------------------|
| **Type O** | Online-install empty-argv product type: empty argv = install-ensure (this product). |
| **Type N** | Non-online-install empty-argv type: empty argv = help — **out of scope** for countdown. |
| **Empty argv / zero-arg** | `$# -eq 0` at entry to `app_main` (no command tokens; classic `curl \| sh` with no trailing args). |
| **Install-ensure** | Converge to “managed `countdown` binary present”; either perform install or success no-op. |
| **Not installed** | `inst_is_installed` returns false (`inst_get_version` → `not installed`). |
| **Installed (local)** | Executable at `${USER_BIN}/countdown` (default `USER_BIN=${HOME}/.local/bin`) observed by install-detect SSOT. |
| **Installed (global)** | Executable at `${GLOBAL_BIN}/countdown` (default `GLOBAL_BIN=/usr/local/bin`) observed by install-detect SSOT. |
...
2. Explicit `countdown help` remains the only full-usage path for help text.  
...
| Invoker | Target |
|---------|--------|
| root (`id -u` 0), e.g. `curl … \| sudo sh` | `${GLOBAL_BIN}/countdown` → `/usr/local/bin/countdown` |
| non-root | `${USER_BIN}/countdown` → `${HOME}/.local/bin/countdown` |
