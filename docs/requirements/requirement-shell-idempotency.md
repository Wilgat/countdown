...
This requirement is the **project Single Source of Truth** for **idempotency (re-run safety)** of state-changing operations in the **POSIX shell CLI** for countdown.
...
A state-changing shell change for countdown is **not done** if any of the following fail:
