...
This requirement is the **project Single Source of Truth** for how the countdown **POSIX shell CLI** behaves in **interactive** (human + TTY) versus **non-interactive** (automation, `curl | sh`, CI/CD, pipes, `--json` / often `--quiet`) environments.
...
Mode-related work for countdown is **not done** if any of the following fail:
